...
#### Step 8: Configure 802.1X on Employee PC (Windows)
...
## 🔒 BONUS: AAA for Device Management Authentication

### Overview
In addition to authenticating **end users** accessing the network (802.1X), we also configured RADIUS authentication for **network administrators** accessing the router and switch management interfaces.

This demonstrates **two different use cases** for RADIUS:
1. **802.1X (already configured):** Controls who can connect to the network
2. **AAA for device management (bonus):** Controls who can manage the devices

### Architecture

**End-User Authentication (802.1X):**
```
Employee PC → Switch Port → RADIUS → Network Access Granted
"Can this DEVICE access the network?"
```

**Admin Authentication (AAA):**
```
Network Admin → SSH to Switch → RADIUS → Management Access Granted
"Can this ADMIN configure the device?"
```

### Real-World Benefit

**Without AAA:**
- 100 network devices = 100 local passwords to manage
...
**RADIUS Server:**
- Added admin users: `netadmin`, `helpdesk`
...
**Fallback:**
- Local user `localadmin` for emergency access if RADIUS down

### Testing Results

✅ SSH to switch requires RADIUS authentication  
✅ Console access requires RADIUS authentication  
✅ Local fallback works if RADIUS unavailable  
✅ Full centralized control over device management

---

### PHASE 5: AAA for Device Management

#### Step 9: Configure AAA for Router/Switch Management
**Objective:** Require RADIUS authentication for SSH/console access to network devices

**On RADIUS Server - Add Admin Users:**

1. Services → AAA
2. Add new users for device management:
...
**Note:** In production, you'd use privilege levels (privilege-level 15 for full access, 1 for read-only)

**On Switch (SW1) - Enable AAA for Management:**
```cisco
! AAA already enabled from Step 5
! (aaa new-model already configured)

! Create method list for login authentication
SW1(config)# aaa authentication login default group radius local

! Enable authorization (who can do what)
...
- `aaa authentication login default group radius local`
    ...
    - Try RADIUS first, fall back to local usernames if RADIUS is down

- `aaa authorization exec default group radius local`

    - RADIUS determines privilege level (what commands you can run)

- `line console 0 / line vty 0 15`

    - Apply AAA to console and SSH access

**On Router (R1) - Same Configuration:**
```cisco
...
**Create Local Fallback Users (Important!):**
**On Switch and Router:**
```cisco
! Create local admin account (in case RADIUS is down)
SW1(config)# username localadmin privilege 15 secret L0c@lAdm1n!

! Create local read-only account
SW1(config)# username readonly privilege 1 secret R3@d0nly!
```
...
### **Testing AAA Device Authentication:**

#### **Test 1: SSH with RADIUS Authentication**

**From any PC (after getting network access via 802.1X):**
```
...
**✅ SUCCESS - Authenticated via RADIUS, got full admin access**
**Verify on switch:**
```cisco
SW1# show users
...
**✅ SUCCESS - Console also requires RADIUS authentication**

#### **Test 3: Fallback to Local User (If RADIUS Down)**
...
SW1#
```
**✅ SUCCESS - Fell back to local authentication** 
**Re-enable RADIUS:**
```cisco
SW1(config)# radius server RADIUS-SRV
SW1(config-radius-server)# address ipv4 10.1.99.50 auth-port 1645
...
### **Test 4: Employee Port (802.1X Authentication)**

**From Employee PC on Fa0/1:**

**Before authentication:**
```
Port Status: Unauthorized
Traffic: BLOCKED (except EAPOL)
```
...
### **Test 6: Compare Authentication vs. No Authentication**

**Side-by-side comparison:**

|Feature |Fa0/1 (Corporate) |Fa0/2 (Guest) |
|---------|---------------|--------------------------|
|802.1X Enabled |✅ Yes |❌ No |
|Authentication Required |✅ Yes (RADIUS) |❌ No (immediate access) |
|VLAN Assignment |10 (Corporate) |20 (Guest) |
|Network Access |Full (after auth) |Internet only (ACL) |
|Use Case |Employees |Visitors, printers |
...
### Problem 3: Authenticated users placed in wrong VLAN
...
### Problem 4: Guest VLAN not working (no fallback)
...
### Problem 5: Authentication works but no network connectivity
...
### Problem 6: Periodic re-authentication disrupts users
