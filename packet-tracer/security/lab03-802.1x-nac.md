...
#### Step 6: Configure 802.1X on Access Ports (Static VLAN Approach)

**Configure Employee port (Fa0/1) - Corporate VLAN:**
```cisco
SW1(config)# interface fa0/1
SW1(config-if)# description Employee PC - 802.1X Enabled
SW1(config-if)# switchport mode access
SW1(config-if)# switchport access vlan 10  ← Authenticated users stay here
...
! Guest VLAN fallback (for failed authentication)
...
**How this works:**
- **Successful auth (john.doe with correct password):** PC stays in VLAN 10 ✅
- **Failed auth (wrong password):** PC moved to VLAN 20 (Guest) ✅
- **No 802.1X support (printer, old device):** PC moved to VLAN 20 (Guest) ✅

**Configure Guest/Test port (Fa0/2):**
```cisco
SW1(config)# interface fa0/2
SW1(config-if)# description Guest PC - 802.1X Enabled
SW1(config-if)# switchport mode access
SW1(config-if)# switchport access vlan 10  ← Starts here
SW1(config-if)# authentication port-control auto
SW1(config-if)# authentication event fail action authorize vlan 20
SW1(config-if)# authentication event no-response action authorize vlan 20
SW1(config-if)# dot1x pae authenticator
...
**Scenario:** Employee PC with valid credentials
...
IP Address: 10.1.10.10  ← Corporate VLAN
Subnet Mask: 255.255.255.0
Default Gateway: 10.1.10.1
```
✅ SUCCESS - Authenticated, stayed in VLAN 10

**Verify on Switch:**
```cisco
ciscoSW1# show authentication sessions interface fa0/1

Interface:  FastEthernet0/1
MAC Address:  0001.9641.2D01
Status:       Authorized
Authorized By:    Authentication Server  ← RADIUS approved
Vlan Policy:      10  ← Still in Corporate VLAN
```
...
**Scenario:** Guest PC with wrong credentials
...
IP Address: 10.1.20.10  ← Guest VLAN
Subnet Mask: 255.255.255.0
Default Gateway: 10.1.20.1
```
✅ SUCCESS - Failed auth, moved to VLAN 20

**Verify on Switch:**
```cisco
ciscoSW1# show authentication sessions interface fa0/2

Interface:  FastEthernet0/2
MAC Address:  0002.1234.5678
Status:       Authorized
Authorized By:    Guest Vlan  ← Failed, fell back to guest
Vlan Policy:      20  ← Guest VLAN
```
✅ Guest VLAN fallback working
...
## 🔧 Packet Tracer vs. Production: Key Differences

| Feature | Packet Tracer | Production (ISE/FreeRADIUS) |
|---------|---------------|----------------------------|
| **Authentication** | ✅ Works | ✅ Works |
| **RADIUS Integration** | ✅ Works | ✅ Works |
| **Guest VLAN Fallback** | ✅ Works | ✅ Works |
| **Dynamic VLAN Assignment** | ❌ Not supported | ✅ Fully supported |
| **RADIUS Attributes** | Username, Password only | 50+ attributes (VLAN, ACL, timeout, etc.) |
| **Role-Based Access** | ❌ Limited | ✅ Full RBAC |

### **Why This Still Matters:**
This lab demonstrates **authentication architecture** and **security policy enforcement**, which are the core concepts. The VLAN assignment mechanism (static vs. dynamic) is an implementation detail that doesn't change the fundamental security model.

In job interviews, I can explain: "I built this in Packet Tracer to learn 802.1X fundamentals, but I understand production systems like Cisco ISE provide dynamic VLAN assignment via RADIUS attributes, which enables true role-based access control."
